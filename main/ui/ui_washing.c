...
static void menu_position_reset() {
    ...
    for (int i = 0; i < FUNC_NUM; i++) {
        ...
        lv_obj_set_style_img_recolor(
            img_funcs[i], lv_color_hsv_to_rgb(200, (40 - abs_t) * 60 / 40, 100),
            0);
        lv_obj_align(img_funcs[i], LV_ALIGN_CENTER, x_axis, y_axis);
        lv_label_set_text_fmt(label_wash_time, "- %02d min -",
                              wash_cycle[item_central].wash_time);
...
static void func_anim_cb(void *args, int32_t v) {
    ...
    for (int i = 0; i < FUNC_NUM; i++) {
        ...
        if (abs_t <= 40) {
            ...
            lv_obj_set_style_img_recolor(
                img_funcs[i],
                lv_color_hsv_to_rgb(200, (40 - abs_t) * 60 / 40, 100), 0);
        }
    }
}
...
static void mask_event_cb(lv_event_t *e) {
    ...
    if (code == LV_EVENT_COVER_CHECK) {
        ...
        lv_draw_mask_radius_init(mask_out_param, &win_area, LV_RADIUS_CIRCLE,
                                 0);
...
void ui_washing_init(lv_obj_t *parent) {
    ...
    lv_obj_set_style_border_color(page_background,
    ...
    lv_obj_set_style_text_font(label_leftTime_unit, &HelveticaNeue_Regular_48,
                               0);
    ...
    lv_obj_set_width(label_wash_time,
    ...
    lv_anim_set_values(&anmi_bub1, lv_obj_get_y_aligned(img_bub1),
                       lv_obj_get_y_aligned(img_bub1) - 90);
    ...
    lv_anim_set_values(&anmi_bub2, lv_obj_get_y_aligned(img_bub2),
                       lv_obj_get_y_aligned(img_bub2) - 90);
    ...
    lv_obj_add_event_cb(page_background, washing_event_cb, LV_EVENT_FOCUSED,
                        NULL);
    lv_obj_add_event_cb(page_background, washing_event_cb,
                        LV_EVENT_LONG_PRESSED, NULL);
    lv_obj_add_event_cb(page_background, washing_event_cb, LV_EVENT_KEY, NULL);
    lv_obj_add_event_cb(page_background, washing_event_cb, LV_EVENT_CLICKED,
                        NULL);
        ...
        switch (wash_mode) {
            ...
            case WASH_MODE_RUN: {
                if (WASH_MODE_STANDBY == wash_mode_xor) {
                    ...
                    lv_label_set_text_fmt(label_leftTimeH, "%d",
                                          (wash_time_left + 59) / 3600);
                    lv_label_set_text_fmt(label_leftTimeL, "%02d",
                                          ((wash_time_left + 59) % 3600) / 60);
                }
            ...
            case WASH_MODE_EOC: {
                ...
                // SOUND_TYPE_WASH_END_CN : SOUND_TYPE_WASH_END_EN);
                // audio_handle_info((LANGUAGE_CN == param->language) ?
                // SOUND_TYPE_WASH_END_CN : SOUND_TYPE_WASH_END_EN);
            } break;
            default:
