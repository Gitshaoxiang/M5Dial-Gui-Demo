...
static ui_menu_app_t menu[] = {
    {"洗衣模式", "Washing", &icon_washing, &icon_washing_ns,
     LV_COLOR_MAKE(36, 163, 235), &washing_Layer},
    {"恒温器", "Thermostat", &icon_thermostat, &icon_thermostat_ns,
     LV_COLOR_MAKE(249, 139, 122), &thermostat_Layer},
    {"照明模式", "Light", &icon_light, &icon_light_ns,
     LV_COLOR_MAKE(255, 229, 147), &light_2color_Layer},
};
...
static void menu_event_cb(lv_event_t *e) {
    ...
    if (LV_EVENT_FOCUSED == code) {
        ...
        if (is_time_out(&time_100ms)) {
            ...
            if ((factory_Enter < 6) && (++factory_Enter == 6) &&
                (app_index == 0)) {
            ...
            lv_img_set_src(icons[get_app_index(0)],
                           menu[get_app_index(0)].icon);
            lv_obj_set_style_border_color(
                page, menu[get_app_index(0)].theme_color, 0);

            // sys_param_t *param = settings_get_parameter();
            // if (LANGUAGE_CN == param->language) {
            //     lv_label_set_text(label_name,
            //     menu[get_app_index(0)].name_CN);
            // } else {
            //     lv_label_set_text(label_name,
            //     menu[get_app_index(0)].name_EN);
            // }
...
void ui_menu_init(lv_obj_t *parent) {
    ...
    lv_obj_set_style_bg_color(tips_btn, lv_color_hex(COLOUR_GREY_4F),
                              LV_PART_MAIN);
    ...
    // if (LANGUAGE_CN == param->language) {
    //     lv_obj_set_style_text_font(tips_label,
    //     &font_SourceHanSansCN_Medium_22, 0); lv_label_set_text(tips_label,
    //     "已恢复出厂");
    // } else {
    ...
    lv_obj_add_event_cb(page, menu_event_cb, LV_EVENT_ALL, NULL);
